...
**Document role**: this file describes how ProFlow is currently built and intended to behave — technical and product architecture only. It is one of three permanent project documents:
...
**Reading order for a new session**: `PROFLOW_PROJECT_CONTEXT.md` → this file → `PROFLOW_HANDOFF.md` → locate the current checkpoint → resume. See `PROFLOW_PROJECT_CONTEXT.md`'s Mandatory Continuity Protocol for the full rule.
...
ProFlow — cloud-based SaaS business-management and quoting platform. Business owners manage clients, create/send price quotes (line items, discount, VAT, digital signature, WhatsApp/email delivery), track expenses/revenue, export reports. **REPO VERIFIED**: React (Vite) frontend, Supabase (Auth, Postgres/RLS, Edge Functions), Vercel hosting, GitHub version control. Cloud-only development workflow — no localhost dev environment used. Modular component design (`Dashboard.jsx` composed of `AdminUsersTab.jsx`, `QuotesTab.jsx`, `SettingsTab.jsx`, `QuoteForm.jsx`, `UserDetailsModal.jsx`, `AILogs.jsx`, `AIChatWidget.jsx`, etc.).
...
**Application-layer state: NOT STARTED.** No `isAdmin`/`isAdminOrAbove` distinction exists in frontend code. No account with `role='admin'` exists. `PROFLOW_TEST_ADMIN_EMAIL`/`PASSWORD` do not exist in `.env` — provisioning is blocked pending owner-chosen credential values (never invented by an agent).
