...
- **Working tree**: see §18.BD below for this checkpoint's exact fresh state (file list, consolidation audit result, and whether a commit/push occurred) — historically NOT clean since the 14.A/14.B/14.C passes began, same 13 application files (`src/pages/Dashboard.jsx`, `src/pages/PublicQuote.jsx`, `src/pages/PublicQuoteEn.jsx`, `src/components/PublicQuoteHeader.jsx`, `src/components/AdminUsersTab.jsx`, `src/components/QuotesTab.jsx`, `src/components/ServicesCatalog.jsx`, `src/components/ClientsTab.jsx`, `src/components/FinancesTab.jsx`, `src/components/SettingsTab.jsx`, `src/components/QuoteForm.jsx`, `src/components/DeleteConfirmModal.jsx`, `src/theme/neonTheme.js`) plus the two documentation files. `PROFLOW_HANDOFF.md`/`PROFLOW_PROJECT_CONTEXT.md`/`PROFLOW_TODO.md` were, until this checkpoint, committed/pushed separately from application code as their own dedicated documentation-only tasks (most recently `bb1d126`) — see §18.BD for whether that separation still holds after this checkpoint's own explicit consolidation authorization.
